...
class LogThread(threading.Thread):
    ...
    def run(self):      
        ...
        # 针对上次拼接，是否需要换行的标志
        split_flag_wrap_2=False
        
        # 追写方式打开日志文件
        # 如果log.txt在同目录下不存在，则自动创建
        # 如果已存在log.txt文件，则往文件里追加内容，不会覆盖
        with open("log"++".txt", "ab",buffering=0) as logfile_a:
            
                # 判断串口是否连接成功
                if ser.is_open:
                    # 循环读取数据
                  while True:
                    buffer_bytes=ser.inWaiting()
                    # 判断数据是否为空                    
                    
                    if buffer_bytes:
                        #print(buffer_bytes)
                        # 读取缓存中所有字节，转化为字符串进行处理                        
                        line_byte_txt=ser.read(buffer_bytes)                       
...
if __name__ == "__main__":     
    ...
    if num >= 0:
        try:
            ...
            ser = serial.Serial(all_port_list[flag].device, baudrate=115200, bytesize=8, stopbits=1, timeout=2)
            print ("串口连接成功")
            #设置为守护线程，防止ctrl+c没有退出的情况
            LogThread(1,all_port_list[flag].device,1).setDaemon("True").start()                                  
            time.sleep(1)
